...
class Query {
  ...
  where (field, comparison, value) {
    this._where.str += ` AND ${this._table}.${field} ${comparison} ?`
  ...
  whereIn (field, values) {
    ...
    this._where.str += ` AND ${this._table}.${field} IN (${marksStr})`
    return this
  }
  ...
  _createInsertQuery () {
    ...
    if (this._insertOnDuplicateKeys.length > 0) {
    ...
    console.log(this._queryStr, this._queryData)
  }
